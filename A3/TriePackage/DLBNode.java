...
public class DLBNode<V> implements TrieNodeInt<V>
{
  ...
  public TrieNodeInt<V> getNextNode(char c){
    ...
    while ((nodelet != null) && (nodelet.cval != c)){
  ...
  public void setNextNode(char c, TrieNodeInt<V> node){
    ...
    if (nodelet.cval != c){
      Nodelet new_nodelet = new Nodelet();
      new_nodelet.cval = c;
      new_nodelet.child = node;
      new_nodelet.rightSib = nodelet.rightSib;
      nodelet.rightSib = new_nodelet;
      degree++;
    }
    else {
      nodelet.child = node;
  ...
  // fine.
  public int getSize(){
    // child ref, sib ref, cval = 10 per degree
    // 4 more bytes for degree
    return 10*degree + 4;
  }
  ...
  public Iterable<TrieNodeInt<V>> children(){
    ...
    while(nodelet != null){
      if (nodelet.child != null){
        q.add(nodelet.child);
        for (TrieNodeInt<V> node: nodelet.child.children()){
          q.add(node);
        }
      }
      nodelet = nodelet.rightSib;
